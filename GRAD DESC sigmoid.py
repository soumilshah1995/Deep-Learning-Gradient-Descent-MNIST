...
for n in range(0, numEpochs):
    ...
    for i in range(trainX.shape[0]):
        ...
        a2 = 1 / (1 + np.exp(-s2))              # A2 = 1 / a + exp(-S2)

        loss += (0.5 * ((a2-trainY[i])*(a2-trainY[i]))).sum()

        # -------------------------------------- BACK PROPOGATE --------------------------------------

        derivative_error_2 = - (trainY[i] - a2)                 # e = Y - A
        derv_act_2 = np.multiply(a2, (1 - a2))                  # derivative of act= A2.(1-A2)
        delta2 = np.multiply(derivative_error_2, derv_act_2)    # Delta2 = -(Y - a).A2.(1 - A2)
